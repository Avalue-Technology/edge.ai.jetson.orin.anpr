...
QDEEP = CDLL('./lib/libQDEEP.so')
QDEEP_COLORSPACE_TYPE_BGR24 = c_ulong(1)
QDEEP_OBJECT_DETECT_CONFIG_MODEL_CUSTOMIZED_LITE = c_ulong(70) 
...
class QuickReceiver():
    def __init__(self):
        ...
        ### AI ###
        strModelName = './model/LPR_LAW_ENFORCEMENT/QDEEP.OD.LICENSE.PLATE.RECOGNITION.LAW.TINY.CFG'
        QDEEP.QDEEP_CREATE_OBJECT_DETECT(QDeepGPUType.NVIDIA, 0 , QDeepObjectDetectConfigModel.QDEEP_OBJECT_DETECT_CONFIG_MODEL_LICENSE_PLATE_RECOGNITION_LAW_ENFORCEMENT, 
                                         strModelName.encode('utf-8'), byref(self.m_detector), QDeepObjectDetectFlag.TRAJECTORY_TRACKING | QDeepObjectDetectFlag.FEATURE_VECTOR)
        QDEEP.QDEEP_START_OBJECT_DETECT(self.m_detector)
        self.m_bStartDetector = 1
        self.m_nWidth = 1920
        self.m_nHeight = 1080
        self.nObjectSize = c_ulong(1000)
        self.PointerArrayType = QDEEP_OBJECT_DETECT_BOUNDING_BOX * self.nObjectSize.value
        self.m_pObjectList = self.PointerArrayType()
        ...
        if video_type == 0:
            video_path = "./model/LPR_LAW_ENFORCEMENT/car.mp4"
             # 確保檔案存在
        ...
        while True:
            ...
            QDEEP.QDEEP_SET_VIDEO_OBJECT_DETECT_UNCOMPRESSION_BUFFER(self.m_detector, QDEEP_COLORSPACE_TYPE_BGR24, c_ulong(self.m_nWidth),
                                                                      c_ulong(self.m_nHeight), pImageBuffer, c_ulong(bufferlen), self.m_pObjectList, byref(self.nObjectSize), 1)
            # print("self.nObjectSize is ", self.nObjectSize)

            for i in range(self.nObjectSize.value):
                    object_color = (0, 0, 255)
                    cv2.rectangle(frame, (self.m_pObjectList[i].nX, self.m_pObjectList[i].nY), (self.m_pObjectList[i].nX+self.m_pObjectList[i].nWidth,
                                                                                                 self.m_pObjectList[i].nY+self.m_pObjectList[i].nHeight), object_color, 3, cv2.LINE_AA)
                    ...
                    for i in range(pointcount):
                        cv2.line(frame, (self.m_nTrajectoryX[i], self.m_nTrajectoryY[i]), (self.m_nTrajectoryX[i + 1], self.m_nTrajectoryY[i + 1]), object_color, 5)
